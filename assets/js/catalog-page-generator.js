(function () {
  ...
  const defaults = {
    property: item => {
      const place = [item.city || 'Jaipur', item.state || 'Rajasthan'].filter(Boolean).join(', ');
      return { title:`${item.title} ${item.property_type === 'Sale' ? 'for Sale' : 'for Rent'} in ${place} | Call4All`, description:`${item.title} available in ${place} for ${money(item.price)}. View property photo, size, video, map and booking details.`, keywords:`${item.title}, property in ${place}, property for ${item.property_type === 'Sale' ? 'sale' : 'rent'}`, imageAlt:`${item.title} property in ${place}` };
    },
    ...
    handmade: item => ({ title:`${item.title} Handmade in ${item.city || 'India'} | Call4All`, description:`Buy ${item.title}, handmade by ${item.artisan || 'a local artisan'}, for ${money(item.price)}. View material, stock and all-India ordering details.`, keywords:`${item.title}, handmade ${item.category}, handmade products India`, imageAlt:`${item.title} handmade by ${item.artisan || 'local artisan'}` })
  };
  ...
  function html(kind, item) {
    const c = config(kind, item), schema = {'@context':'https://schema.org','@type':kind === 'property' ? 'RealEstateListing' : 'Product',name:item.title,description:c.description,image:[c.image],url:c.canonical,sku:item.id,offers:{'@type':'Offer',price:String(item.price || ''),priceCurrency:'INR',availability:'https://schema.org/InStock',url:c.canonical}};
    if (kind === 'property') Object.assign(schema,{datePosted:item.timestamp,address:{'@type':'PostalAddress',addressLocality:item.city,addressRegion:item.state,addressCountry:'IN'},geo:{'@type':'GeoCoordinates',latitude:item.latitude,longitude:item.longitude}});
    else schema.itemCondition = kind === 'second-hand' ? 'https://schema.org/UsedCondition' : 'https://schema.org/NewCondition';
...
<meta property="og:type" content="product"><meta property="og:site_name" content="Call4All"><meta property="og:locale" content="en_IN"><meta property="og:title" content="${esc(c.ogTitle)}"><meta property="og:description" content="${esc(c.ogDescription)}"><meta property="og:url" content="${esc(c.canonical)}"><meta property="og:image" content="${esc(c.image)}"><meta property="og:image:alt" content="${esc(c.imageAlt)}"><meta property="product:price:amount" content="${esc(item.price)}"><meta property="product:price:currency" content="INR">
