(function(){
  ...
  const thaliAd=()=>`<aside class="property-thali-ad" aria-label="Call4Thali"><a class="property-thali-image-link" href="tiffin-center-kukas.html"><img src="assets/uploads/tiffin-center-kukas-banner.png" alt="Call4Thali fresh vegetarian meals from Rs 49" loading="lazy"></a><div class="property-thali-copy"><h2>Fresh vegetarian thali from Rs 49</h2><p>Enjoy a tasty meal after your property visit. Delivery available in Kukas, Jaipur.</p><div class="property-thali-actions"><a class="btn btn-accent btn-sm" href="tiffin-center-kukas.html">View Menu</a><a class="btn btn-whatsapp btn-sm" href="https://wa.me/917737353588?text=${encodeURIComponent('Hello Call4Thali, I would like to order a thali.') }" target="_blank" rel="noopener">Order on WhatsApp</a></div></div></aside>`;
  ...
  function card(p){return `<article class="property-list-card"><a href="${detailUrl(p)}"><img src="${safe(image(p.image_path))}" alt="${safe(p.title)} property ${safe(type(p))} in ${safe(locationName(p))}" loading="lazy"></a><div class="property-list-body"><span class="property-type type-${type(p)==='Sale'?'sale':'rent'}">${typeIcon(p)} ${safe(type(p))}</span><h3><a href="${detailUrl(p)}">${safe(p.title)}</a></h3><h2 class="property-location-tag">📍 ${safe(locationName(p))}</h2><strong class="property-card-price">${money(p.price)}</strong><p>📐 ${safe(p.width_ft)} × ${safe(p.height_ft)} ft · ${Number(p.width_ft)*Number(p.height_ft)||0} sq ft</p><div class="property-card-actions"><a class="property-view-link" href="${detailUrl(p)}">View details →</a>${videoButton(p)}<a class="btn btn-property-call btn-sm" href="${callUrl()}">☎ Call</a><a class="btn btn-whatsapp btn-sm" href="${whatsappUrl(p)}" target="_blank" rel="noopener">WhatsApp</a></div></div></article>`}
  function addItemListSchema(list){const s=document.createElement('script');s.type='application/ld+json';s.textContent=JSON.stringify({'@context':'https://schema.org','@type':'ItemList',name:'Properties for Sale and Rent Across India',numberOfItems:list.length,itemListElement:list.map((p,i)=>({'@type':'ListItem',position:i+1,item:{'@type':'RealEstateListing',name:p.title,url:'https://call4all.co.in/'+detailUrl(p),image:[absoluteUrl(p.og_image||p.image_path,'assets/icons/icon-512.png')],description:p.meta_description||p.description,offers:{'@type':'Offer',price:String(p.price||''),priceCurrency:'INR',availability:'https://schema.org/InStock',url:'https://call4all.co.in/'+detailUrl(p)},address:{'@type':'PostalAddress',addressLocality:p.city||'Jaipur',addressRegion:p.state||'Rajasthan',addressCountry:'IN'}}}))});document.head.appendChild(s)}

  function priceValue(value){const normalized=String(value||'').replace(/[^\d.]/g,'');const parsed=Number(normalized);return Number.isFinite(parsed)&&parsed>=0?parsed:null}
  function priceMatches(item,minPrice,maxPrice){const price=Number(item.price)||0;if(minPrice!==null&&price<minPrice)return false;if(maxPrice!==null&&price>maxPrice)return false;return true}
  async function mapPage(){
    const el=document.getElementById('propertyMap');if(!el)return;
    let thaliMount=document.getElementById('propertyThaliAd');if(!thaliMount){thaliMount=document.createElement('div');thaliMount.id='propertyThaliAd';document.querySelector('.property-list-section')?.before(thaliMount)}thaliMount.innerHTML=thaliAd();
    if(!window.L)return;
    ...
    if(list.length){const social=absoluteUrl(list[0].og_image||list[0].image_path,'assets/icons/icon-512.png');setMeta('meta[property="og:image"]','property',social);setMeta('meta[property="og:image:secure_url"]','property',social);setMeta('meta[name="twitter:image"]','name',social);}
    list.forEach(p=>{
  ...
  async function detailPage(){
    ...
    const seoLocationText = locationName(p);
    ...
    setMeta('meta[name="description"]','name',desc.includes(seoLocationText) ? desc : `${desc} Location: ${seoLocationText}.`);
    setMeta('meta[name="keywords"]','name',(p.seo_keywords||`${p.title}, ${type(p)} property in ${seoLocationText}, ${seoLocationText} near me, property near ${seoLocationText}, ${p.city || 'Jaipur'} ${p.state || 'Rajasthan'}`).trim());
    ...
    mount.insertAdjacentHTML('afterbegin',thaliAd());bindVideoButtons(mount);
